# Fork tracking — intarweb/glance

This is a soft fork of [`glanceapp/glance`](https://github.com/glanceapp/glance). We track upstream HEAD and publish a HEAD-tracked image to GHCR with any open intarweb→upstream PRs cherry-picked on top.
...
| Property | Value |
|---|---|
| Upstream | [`glanceapp/glance`](https://github.com/glanceapp/glance) |
...
services:
  glance:
    image: ghcr.io/intarweb/glance:latest
```

Pin to a specific build for reproducibility:

```yaml
    image: ghcr.io/intarweb/glance:sha-<short>
```
...
```bash
gh workflow run "Sync from upstream + auto-regen intarweb-dev" --repo intarweb/glance
```

**Force a fresh build of `:latest` without an upstream change:**
```bash
gh workflow run "Build from source → GHCR" --repo intarweb/glance --ref intarweb-dev
```
...
gh pr create --repo glanceapp/glance --base main --head intarweb:feat/my-thing
